...
def _get_or_start_client():
    ...
    with _client_lock:
        ...
        try:
            ...
            # Primary: Synthesis REST API gives us all ~42k open-interest markets
            # plus event groupings needed for ME / threshold scanning.
            # Falls back to Kalshi volume-filtered list if no Synthesis key.
            # force_refresh=True bypasses the 6-hour cache so restarts always
            # subscribe to the freshest ticker list (new markets opened since
            # last cache write are included immediately).
            ...
            def _arb_scanner_thread(state=state, cache=cache, event_groups=event_groups):
                ...
                _last_evict_ts = 0.0
                _EVICT_INTERVAL_S = 600  # evict stale markets every 10 minutes
                ...
                while True:
                    _time.sleep(1)   # scan every 1s for arb opportunities
                    _scan_count += 1
                    global _scanner_cycles
                    _scanner_cycles += 1
                    try:
                        # Periodic memory eviction: remove stale/closed markets.
                        # SKIP eviction while the WS feed is silent — stale reference
                        # data is more useful than an empty cache.  Only evict when the
                        # feed is actively delivering updates (last message < 120s ago).
                        _now = _time.time()
                        if _now - _last_evict_ts > _EVICT_INTERVAL_S:
                            _last_msg_age = _now - (state.get_stats().get("last_message_ts") or 0)
                            if _last_msg_age < 120:
                                _evicted = state.evict_stale_markets()
                                if _evicted:
                                    logger.info("LiveState eviction: removed %d stale markets", _evicted)
                            else:
                                logger.info(
                                    "LiveState eviction skipped — WS silent for %.0fs, "
                                    "preserving %d cached markets",
                                    _last_msg_age, len(state.snapshot_all()),
                                )
                            # Evict stale _last_seen entries to prevent unbounded growth
                        ...
                        # Heartbeat every 60 scans (~1 min)
                        if _scan_count % 60 == 0:
                            _hb_q = len(state.snapshot_all())
                            _hb_b = len(cache.snapshot())
                        ...
                        _quotes = state.snapshot_all()
                        if not _quotes:
                            continue
                        _books = cache.snapshot()
            ...
            _scanner.start()
            logger.info("Background arb scanner started (1s scan interval, 5 strategies)")
